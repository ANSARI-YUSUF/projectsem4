...
if ($_FILES['file']['error'] === UPLOAD_ERR_OK) {
    ...
    $query3='select count(gfid)as co from group_files where G_ID="'.$gid.'" and gfile_name="'.$fileName.'"'; 
    $result3=mysqli_query($conn,$query3);
    $record=mysqli_fetch_assoc($result3);

    if($record["co"]>=1){
        echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">Rename your File
        <a href="groupdesktop.php">Back</a></div>
        ';
        exit();
    }
    ...
    if (move_uploaded_file($fileTmpPath, $destPath)) {
        ...
        $query2="INSERT INTO `group_files` ( `gfile_name`,  `gfile_type`, `gfile_path`,`gfile_size`,`U_ID`,`G_ID`) VALUES ('$fileName', '$fileType', '$destPath','$fileSize',$uid,$gid)";
        $result2=mysqli_query($conn,$query2);
        if(!$result2){
